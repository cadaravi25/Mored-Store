...
select p.numero_externo,
 ...
 order by p.numero_externo;

-- --- 2. Lineas de compra vs prendas vendibles ---
select p.numero_externo,
       count(distinct l.id)                        as lineas_compra,
       sum(l.cantidad_pedida * fn_piezas_por_pack(l.id)) as prendas_vendibles
  from pedidos_compra p
  join pedidos_compra_lineas l on l.pedido_id = p.id
 group by p.id, p.numero_externo
 order by p.numero_externo;

-- --- 3. Costo landed: esperado 10,69 el top y 7,12 la prenda del set ---
select pr.nombre as producto, col.nombre as color, v.talla,
       v.stock, v.costo_promedio_usd
  from variantes v
  join productos pr on pr.id = v.producto_id
  join colores col  on col.id = v.color_id
 where v.stock > 0
 order by v.costo_promedio_usd desc, pr.nombre, col.nombre;

-- --- 4. Que sigue pendiente por llegar ---
select numero_externo, producto, color, talla,
       piezas_faltantes, monto_faltante_usd, dias_esperando
  from v_prendas_pendientes
 order by dias_esperando desc, producto, color;

-- --- 5. El courier quedo registrado como egreso ---
select tipo, concepto, categoria, monto_usd, cuenta
  from movimientos_financieros
 order by creado_at desc;

-- ============================================================================
-- LIMPIEZA (descomentar para borrar todo lo de prueba)
-- ============================================================================
-- begin;
--   delete from movimientos_financieros where origen = 'compra';
