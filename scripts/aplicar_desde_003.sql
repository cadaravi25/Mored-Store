-- Mored Store: migraciones 003 a 007.
-- 001 y 002 ya estan aplicadas. La 003 fallaba por orden de dependencias y ya esta corregida.

do $guard$
begin
  if to_regclass('public.variantes') is null then
    raise exception 'Falta la migracion 001.';
  end if;
  if not exists (select 1 from pg_proc where proname = 'f_normalizar') then
    raise exception 'Falta la migracion 002.';
  end if;
  if to_regclass('public.pedidos_compra_lineas_componentes') is not null then
    raise exception 'La migracion 003 ya estaba aplicada. Aplica solo de la 004 en adelante.';
  end if;
end
$guard$;
